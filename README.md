...
- 🖥️ **官方 slot 工作台**：`shell.overlay` 驱动的左上角悬浮按钮通过 body portal 落在对话界面层，右侧抽屉（主会话/子代理/技能设置/预设配置/角色管理五页）仍由官方 slot 承载；按钮纵向位置独立于其他插件，`sidebar.footer.action` 几何探针仅跟随官方侧栏的 264px 起步、可拉伸及 56px 折叠宽度，并保留 20px 横向间距；UI 挂载全部交给官方 SlotRegistry，无宿主 DOM 选择器
