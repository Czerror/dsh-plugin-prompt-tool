...
- **独立 agent preset 层**：插件加载时直引 anchored-standard 上游文件生成 preset 到 `~/.dsh/.agent-presets/prompt-tool/`（首轮 = 官方 Minimal 真实 schema：持久 `bash` + `str_replace_editor` + 剥离自动注入上下文，无输出 cap）；首轮 reasoning 稳定 "we" 轨迹，we 锚定确认后注入 `prompt.md`，随后放开完整 Standard 目录。
...
| 项目 | 关系 | 复用内容 |
|---|---|---|
| [dsh-anchored-standard](https://github.com/xiaobright/dsh-anchored-standard) | **集成（上游）** | 加载时直引子模块 `vendor/dsh-anchored-standard` 的 `agent.cordis.yml` + `tool-bootstrap.mjs`（更新即生效），Minimal 真实 schema 首轮锚定机制 |
...
上游文件不复制、不锁版本、无同步步骤：插件加载（写 preset）时直接读子模块
`vendor/dsh-anchored-standard/preset/` 里的 `agent.cordis.yml`（运行时注入
prompt-injector 块与 `prompt.md`）与 `tool-bootstrap.mjs` 最新内容。
`git submodule update` 后重启 dsh 即生效。
