...
meta:
  contentType: Archive
  title: 模块列表与引擎设置归一重构计划（已完成）
  category: Engineering
---

# 模块列表与引擎设置归一重构计划（历史归档）

> 归档说明：本文原为 `PLAN.md` §15 的折叠内容。计划于 2026-09-08 更新、2026-09-09 随 `dev` 提交完成后归档到 `.scratch/prompt-tool-framework/`。本文只保留历史设计、验收清单和当时的验证记录，**不是当前待办**，也不代表任何待办状态；当前计划以 [`PLAN.md`](../../../PLAN.md) 为准。
...
- 落地时同步 [UI 权威文档](../../../docs/ui-architecture.md) 与 [框架 spec](../spec.md) 的布局、按需请求和验收描述；本次不同时改写 spec。
