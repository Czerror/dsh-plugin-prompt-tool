...
### 2026-09-20 复核补录（补齐 G1—G7，不改变上文结论）

按用户要求「继续完成未完成的重构项」并再次核对本计划。先澄清唯一误导源：上文「本轮修复验收（Wave 1+2 / T2—T6）」末尾那句 `未实施：Wave 3、4、6—9 全部任务` 是 **Wave 1+2 当轮的边界记录**，随后已由 `54bac00`（Wave 3 + T13）、`b85935e`、`d72f624`（Wave 7—9 并归档本计划）完成；「未跑性能基准」与「不重启运行中的 DSH 服务」是本计划显式的不采纳项与仓库硬约束，不是待办。

对照上文「验收标准」逐条复核后，仍有 G1—G7 七项未真正落地，本轮已补齐并推送 `origin/dev`：

| 缺口 | 本轮结论与证据 |
|---|---|
| G1 九层编辑面只有 36 行、页面仍各自拼装卡片 | `EngineLayersPanel#engineLayerSlots` 成为层内卡片唯一装配入口，两页只声明受众视图、搜索词与页面编排（`e5ce012`） |
| G2 tool-pipeline 能力区只放了两个控件 | 共享设置区按真实能力 card 分八组（六个本层能力组 + 两个登记的跨层相关设置），参数键仍由 shared 派生（`e5ce012`） |
| G3 跨层镜像与共享同步语义缺失 | store 新增草稿广播通道，未完成输入与错误态在两个渲染点实时同步、一次失焦只保存一次，真实 Edge smoke 覆盖（`a5affe0`） |
| G4 合成 128/64 卡往返回归缺失 | 184 卡（pre-step 120 + system-section 64）渲染与逐卡操作只作用目标实例；host 落盘读回保真（`abf5781`） |
| G5 搜索未覆盖能力名/技术键、层空态缺失 | 统一搜索覆盖配置名、标识、注入层中文名与参数名；层内无内容给空状态与新增入口（`401bcab`） |
| G6 受管字段只给一句提示 | shared `managed-config-fields` + 实例卡只读来源面板 + host 投影一致性回归（`78705db`） |
| G7 各 owner 端到端与切层草稿保持 | 切层/受众草稿保持回归（`abf5781`）；各存储 owner 的保存链路由既有 settings-bridge / write-preset / instruction-save-flow / preset-render-variants 用例覆盖 |

**本次未做（如实记录，不宣称已完成）**：受管字段在实例卡内是只读回显与来源标注，没有做成跨 feature 的可写镜像控件（features 之间不互相 import 内部实现，写入口保持在提示词生成默认值卡与共享设置区）；未运行性能基准，不宣称延迟、吞吐或资源消耗改善；未重启或抢占运行中的 DSH 服务。
