...
test('rematerialize-presets：dry-run 只报告不写盘', () => {
  ...
  try {
    ...
    assert.match(output, /would materialize creative/)
    assert.match(output, /would refresh skills creative/)
...
test('rematerialize-presets：刷新保留独有文件和目录，只有 extra 时不重复备份', () => {
  ...
  try {
    ...
    assert.match(run(home, '--refresh-skills'), /refreshed skills creative/)
  ...
  try {
    ...
    const result = runWithFsMock(home, `
      ...
      mock.method(fs, 'cpSync', (source, target, options) => {
        if (source === ${JSON.stringify(join(ROOT, 'preset', 'creative', 'skills'))}) {
...
test('rematerialize-presets：刷新不沿同名目录链接外写，独有链接和旧链接保留', () => {
  ...
  try {
    ...
    writeFileSync(join(dir, 'preset.yml'), 'name: creative\norder: 0\n', 'utf8')
    const skills = join(dir, 'skills')
