 ...
 *  2. 状态文件读失败不得回落默认状态——否则一个瞬时坏文件会把屏蔽表与引用目录一起清空；
 *  3. 只有状态确实变化才失效候选缓存，普通文件事件不让官方 skill 提供者跟着全量重扫。
 *
 *  断言全部落在可观察输出上：accept 收到的状态、各依赖的调用次数、告警文本与降级标志。 */
...
test('状态文件变化：替换状态、重挂 watcher 并失效清单与候选缓存', () => {
  ...
  assert.equal(reloader.degraded(), false)
})

test('状态未变（引用目录里的文件变化）也必须失效清单缓存，但不失效候选缓存', () => {
  const { file } = makeStateFile()
  ...
  assert.equal(calls.invalidateCandidates, 0, '候选集合没变，不让官方提供者跟着全量重扫')
})
...
test('状态文件损坏：保留上一次有效状态、只告警一次，修好后自动恢复', () => {
  ...
  assert.equal(reloader.degraded(), true)
  ...
  reloader.reload()
  assert.equal(reloader.degraded(), false, '文件修好后退出降级状态')
  assert.equal(calls.accepted.length, 1, '恢复后接受新的合法状态')
  assert.deepEqual(calls.accepted[0].blocked, [])
  assert.equal(calls.invalidateCandidates, 1, '恢复后状态变化才失效候选缓存')
})
...
test('真实装配：状态文件写入经 watcher 走完整条刷新链路', async () => {
  ...
  try {
    ...
    assert.equal(calls.accepted.length, 1, 'watcher 事件必须走完整个刷新装配')
    assert.deepEqual(calls.accepted[0].blocked.map((item) => item.name), ['demo'])
