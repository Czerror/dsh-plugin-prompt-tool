...
test('浏览器：能力创建未完成时切换等待，移除真实生效且失败不阻塞队列', { skip: skipBrowser, timeout: 30000 }, async () => {
  ...
  await sleep(100)
  assert.deepEqual(await evaluate('window.presetSwitches'), [], '切换必须等待能力写入及其刷新完成')
  await evaluate('window.delayCapabilities=false; window.pendingCapabilities.shift()(); true')
  await waitFor('window.createDone && window.switchDone')
