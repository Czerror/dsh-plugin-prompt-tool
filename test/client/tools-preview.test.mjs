...
test('编辑保留在主会话，工具预览没有保存或安装管理入口', () => {
  ...
  assert.match(edit, /从模板新建/)
  assert.match(edit, /新建工具/)
...
test('自定义工具按预设 key 重挂载，system 或关闭 writePreset 时禁用整个编辑区', () => {
  ...
  assert.match(main, /disabled=\{store\.moduleFacts\?\.editable !== true \|\| !fields\.writePreset\}/)
  ...
  for (const action of ['从模板新建', '新建工具', 'onClick={save}', '<CustomToolCard']) assert.ok(fieldset.includes(action), `${action} 必须在原生禁用边界内`)
  assert.match(source, /pickerOpen && !disabled &&/)
  assert.match(source, /const save = \(\): void => \{\s+if \(disabled \|\| saving\) return/)
  assert.match(source, /const insertTemplate = \(spec: ToolDraft\): void => \{\s+if \(disabled\) return/)
