...
test('浏览器：六层空卡、跨层工具创建、筛选草稿与行为下拉', { skip: !existsSync(browserPath) && '设置 PROMPT_TOOL_TEST_BROWSER 以运行真实浏览器回归', timeout: 60000 }, async () => {
  ...
  try {
    ...
    // 指令文件（AGENTS.md）只在前置步骤层卡内：列表不出独立卡片，正文在层卡里编辑。
    assert.equal(await evaluate(`[...document.querySelectorAll('[class="configName"]')].some((e) => e.textContent.startsWith('AGENTS：'))`), false)
    await evaluate(`document.querySelector('[aria-label="编辑行为"]').click()`)
    await sleep(80)
    assert.match(await evaluate('document.body.innerText'), /AGENTS：~\/\.dsh\/AGENTS\.md/)
    await click('AGENTS：AGENTS.md')
    await waitFor(`document.querySelector('[aria-label="注入内容（空 = 不注入）"]')?.value==='项目指令正文'`)
    assert.equal(await evaluate(`document.querySelector('[aria-label="启用 AGENTS：AGENTS.md"]')!==null`), true)
