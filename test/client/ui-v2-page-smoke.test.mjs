...
test('V2 页面：导航、浏览恢复、配置筛选与保存反馈', { skip: !existsSync(browserPath) && '设置 PROMPT_TOOL_TEST_BROWSER 指向 Edge/Chrome 可执行文件', timeout: 90000 }, async () => {
  ...
  await clickText('校验')
  await waitFor(`document.querySelector('[data-workspace-notice]')?.innerText.includes('配置校验通过')`)
  await evaluate('window.rejectSave=true')
  await click('[data-module-toolbar] .primaryPill')
  await waitFor(`/尚未保存|fixture save failed/.test(document.querySelector('[data-workspace-notice]')?.innerText ?? '')`)
  assert.equal(await evaluate(`document.querySelectorAll('[role="status"]').length>0`), true)
  await evaluate('window.rejectSave=false')
  await click('[data-module-toolbar] .primaryPill')
  await waitFor(`document.querySelector('[data-workspace-notice]')?.innerText.includes('已保存')`)
...
test('技能卡：正文与描述分开编辑、草稿恢复、冲突与重读确认、在途输入和可编辑边界', { skip: !existsSync(browserPath) && '设置 PROMPT_TOOL_TEST_BROWSER', timeout: 30000 }, async () => {
  const { evaluate, waitFor, click, inputCard, inputDraft, navigate, count } = await ensureSession()
  await navigate('drafts')
  await waitFor('window.store?.getFields().skillCatalog.length===2')
  await click('[data-page="skills"]')
  await waitFor('document.querySelectorAll("[data-skill-card]").length===2')
  assert.equal(await evaluate(count('skill-read')), 0, '折叠列表不读取技能正文')
  const alpha = '[data-skill-card="user-dsh:D:/isolated/skills:alpha"]'
  const editor = `${alpha} [data-skill-editor]`
  const editorValue = `document.querySelector(${JSON.stringify(editor)})?.value`
  const descriptionEditor = `${alpha} [data-skill-description-editor]`
  const descriptionValue = `document.querySelector(${JSON.stringify(descriptionEditor)})?.value`
  const alphaPath = 'D:/isolated/skills/alpha/SKILL.md'
  await click(`${alpha} [data-skill-expand]`)
  await waitFor(`${editorValue}?.includes('Original skill body.')`)
  assert.deepEqual(await evaluate(`window.requests.find(r=>r.endpoint==='skill-read').body`), { name: 'alpha', path: alphaPath })
  const original = await evaluate(editorValue)
  assert.equal(original, '# alpha\nOriginal skill body.\n', '内容编辑器只展示 Markdown 正文')
  assert.doesNotMatch(original, /^(?:---|name:|description:|disable-model-invocation:|user-invocable:)/m, '名称、描述与调用策略 frontmatter 不进入正文')
  assert.equal(await evaluate(descriptionValue), 'alpha description', '描述在独立输入框内显示')
  await inputCard(descriptionEditor, 'saved description')
  assert.equal(await evaluate(`document.querySelector(${JSON.stringify(alpha + ' [data-skill-save]')}).disabled`), false, '只改描述也能保存')
  const submitted = original.replace('Original skill body.', 'Submitted body.')
  await inputCard(editor, submitted)
  await click(`${alpha} [data-skill-expand]`)
  await click(`${alpha} [data-skill-expand]`)
  assert.equal(await evaluate(editorValue), submitted, '折叠保留草稿')
  await click('[data-page="tools"]')
  await click('[data-page="skills"]')
  assert.equal(await evaluate(editorValue), submitted, '切页恢复展开和草稿')
  assert.equal(await evaluate(descriptionValue), 'saved description', '切页保留独立描述草稿')
  await inputDraft('skills.filter.aria', 'beta')
  assert.equal(await evaluate(`document.querySelector(${JSON.stringify(alpha)})===null`), true)
  await inputDraft('skills.filter.aria', '')
  assert.equal(await evaluate(editorValue), submitted, '过滤卸载后恢复草稿')
  assert.equal(await evaluate(descriptionValue), 'saved description', '过滤卸载后恢复描述草稿')
  assert.equal(await evaluate(count('skill-read')), 1, '恢复草稿不重复读取覆盖输入')

  await evaluate('window.rejectSkillWrite=true')
  await click(`${alpha} [data-skill-save]`)
  await waitFor(`document.querySelector(${JSON.stringify(alpha)})?.querySelector('[role="alert"]')!==null`)
  assert.equal(await evaluate(editorValue), submitted, '冲突保留正文草稿')
  assert.equal(await evaluate(descriptionValue), 'saved description', '冲突保留描述草稿')
  assert.deepEqual(await evaluate(`window.requests.find(r=>r.endpoint==='skill-write').body`), { name: 'alpha', path: alphaPath, content: submitted, description: 'saved description', expectedRevision: 'skill-1' })
  await click(`${alpha} [data-skill-reload]`)
  await waitFor('document.querySelector("[role=alertdialog]")!==null')
  assert.equal(await evaluate(count('skill-read')), 1, '确认前不重读')
  await click('[role="alertdialog"] button:not([data-danger])')
  assert.equal(await evaluate(editorValue), submitted, '取消重读保留草稿')
  assert.equal(await evaluate(descriptionValue), 'saved description', '取消重读保留描述草稿')
  await click(`${alpha} [data-skill-reload]`)
  await click('[role="alertdialog"] button[data-danger]')
  await waitFor(`${editorValue}===${JSON.stringify(original)}`)
  assert.equal(await evaluate(descriptionValue), 'alpha description', '确认重读恢复磁盘描述')
  assert.equal(await evaluate(count('skill-read')), 2, '确认后才恢复磁盘版本')

  await inputCard(editor, submitted)
  await inputCard(descriptionEditor, 'saved description')
  await evaluate('window.rejectSkillWrite=false;window.skillWriteDelay=400')
  const refreshBefore = await evaluate(count('skills-list'))
  await click(`${alpha} [data-skill-save]`, 10)
  await waitFor(`${count('skill-write')}===2`)
  const later = submitted.replace('Submitted body.', 'Later unsaved body.')
  await inputCard(editor, later)
  await inputCard(descriptionEditor, 'later unsaved description')
  await waitFor(`${count('skills-list')}>${refreshBefore} && window.store.skillsBusy===false`)
  assert.equal(await evaluate(editorValue), later, '保存成功保留在途新输入')
  assert.equal(await evaluate(descriptionValue), 'later unsaved description', '保存成功保留在途描述输入')
  assert.equal(await evaluate(`window.skillFiles.get(${JSON.stringify(alphaPath)}).content`), submitted, '请求只保存点击时的文本')
  assert.equal(await evaluate(`window.skillFiles.get(${JSON.stringify(alphaPath)}).description`), 'saved description', '请求只保存点击时的描述')
  assert.equal(await evaluate(`document.querySelector(${JSON.stringify(alpha)})?.querySelector('[data-skill-description]').textContent`), 'saved description', '保存成功刷新清单元数据')
  assert.equal(await evaluate(`document.querySelector(${JSON.stringify(alpha + ' [data-skill-save]')}).disabled`), false, '更新基线后仍可保存新输入')
  await evaluate('window.skillWriteDelay=0')
  await click(`${alpha} [data-skill-save]`)
  await waitFor(`window.store.skillsBusy===false && window.skillFiles.get(${JSON.stringify(alphaPath)}).content===${JSON.stringify(later)}`)
  assert.equal(await evaluate(`window.requests.filter(r=>r.endpoint==='skill-write').at(-1).body.expectedRevision`), 'skill-2', '后续保存使用新的版本')
  assert.equal(await evaluate(`window.skillFiles.get(${JSON.stringify(alphaPath)}).description`), 'later unsaved description', '再次保存提交更新后的描述')

  // 读取失败不创建可覆盖文件的空白基线；无效文件可以修复，官方只读条目仍无编辑入口。
  await evaluate(`(()=>{const base=window.getSkills()[0];window.setSkills([{...base,id:'broken',name:'broken',description:'',path:'D:/workspace/.dsh/skills/broken/SKILL.md',valid:false,issue:'missing description',canEdit:true,canSetPolicy:false},{...base,id:'readonly',name:'readonly',path:'D:/system/skills/readonly/SKILL.md',canEdit:false,canSetPolicy:false}]);window.rejectSkillRead=true;return window.store.refreshSkills()})()`)
  await click('[data-skill-card="broken"] [data-skill-expand]')
  await waitFor('document.querySelector("[data-skill-card=broken] [role=alert]")!==null')
  assert.equal(await evaluate(`document.querySelector('[data-skill-card="broken"] [data-skill-save]')?.disabled ?? true`), true, '读取失败禁止盲写')
  await evaluate('window.rejectSkillRead=false')
  await click('[data-skill-card="broken"] [data-skill-reload]')
  await waitFor('document.querySelector("[data-skill-card=broken] [data-skill-editor]")?.value.includes("Original skill body.")')
  assert.equal(await evaluate(`document.querySelector('[data-skill-card="broken"] [data-skill-editor]').readOnly`), false, '无效条目仍可修改正文与描述')
  await inputCard('[data-skill-card="broken"] [data-skill-description-editor]', 'repaired description')
  await inputCard('[data-skill-card="broken"] [data-skill-editor]', 'Repaired body.')
  await click('[data-skill-card="broken"] [data-skill-save]')
  await waitFor('window.store.getFields().skillCatalog.find(s=>s.id==="broken").valid===true')
  const readsBeforeReadonly = await evaluate(count('skill-read'))
  await click('[data-skill-card="readonly"] [data-skill-expand]')
  assert.equal(await evaluate('document.querySelector("[data-skill-card=readonly] [data-skill-editor]")===null'), true)
  assert.equal(await evaluate(count('skill-read')), readsBeforeReadonly, '只读条目不调用编辑读取接口')

  await click('[data-page="tools"]')
  await evaluate(`window.currentSessionId='second-session';window.setSkills(window.getSkills().map(s=>({...s,id:'session-skill',name:'session-skill',canEdit:true,valid:true})).slice(0,1));window.store.refreshSkills()`)
  await waitFor('window.store.getFields().skillCatalog[0]?.name==="session-skill"')
  await click('[data-page="skills"]')
  await click('[data-skill-card="session-skill"] [data-skill-expand]')
  await waitFor('document.querySelector("[data-skill-card=session-skill] [data-skill-editor]")?.value.length>0')
  assert.equal(await evaluate(`window.requests.filter(r=>r.endpoint==='skill-read').at(-1).body.sessionId`), 'second-session')
  await inputCard('[data-skill-card="session-skill"] [data-skill-description-editor]', 'scoped description')
  await inputCard('[data-skill-card="session-skill"] [data-skill-editor]', 'Scoped body.')
  await click('[data-skill-card="session-skill"] [data-skill-save]')
...
test('V2 草稿与资源：原文恢复、快照保存、技能目标及危险删除', { skip: !existsSync(browserPath) && '设置 PROMPT_TOOL_TEST_BROWSER 指向 Edge/Chrome 可执行文件', timeout: 90000 }, async () => {
  ...
  await click('[data-page="skills"]')
  await waitFor(`document.querySelectorAll('[data-skill-card]').length===2`)
  assert.equal(await evaluate(`[...document.querySelectorAll('[data-skill-card] [role="switch"]')].filter(e=>e.getClientRects().length).length`), 0, '技能默认折叠，调用策略在卡内展开')
  const expandSkill = async (name) => {
    await evaluate(`(()=>{const skill=window.store.getFields().skillCatalog.find(s=>s.name===${JSON.stringify(name)});const card=[...document.querySelectorAll('[data-skill-card]')].find(e=>e.dataset.skillCard===skill.id);const button=card.querySelector('[data-skill-expand]');if(button.getAttribute('aria-expanded')==='false')button.click()})()`)
    await sleep(60)
  }
  await expandSkill('alpha')
  await expandSkill('beta')
  ...
  await expandSkill('custom-helper')
  await expandSkill('user-helper')
