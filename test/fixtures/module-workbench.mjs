...
import { engineCapability, engineRecipe, engineGroupParamKeys, impliedModulesForParams } from '../../src/shared/engine-capabilities.ts'
...
const api = { sessionModel: { snapshot: () => session, subscribe: () => () => {}, select: async (selection) => { window.sessionSelection = selection } }, sessionPreset: { snapshot: () => undefined, subscribe: () => () => {} }, currentSessionId: () => undefined, subscribeSessionChange: () => () => {},
  switchPreset: async id => {
    ...
    ;({ configs, variables, tools, overrides, modules } = presets.get(id) ?? { configs: [], variables: {}, tools: [], overrides: {}, modules: new Set() })
    presetId = id
...
window.fetch = async (url, init) => {
  ...
  if (endpoint === 'bootstrap') return new Response(JSON.stringify({ ok: true,
    value: { value: { presetTemplate: presetId, writePreset: window.writePreset !== false }, base: {}, revision: 1 },
    ...
    moduleFacts: { sourceMode: 'explicit', editable: window.presetEditable !== false, effectiveModules: [...new Set([...modules, ...impliedModulesForParams(overrides, {})])], declaredModules: [...modules], rowIds: [] },
  }))
  ...
  if (endpoint === 'param-overrides') {
    if (window.delayOverrides) await new Promise(resolve => window.pendingOverrides.push(resolve))
    if (body.expectedPresetId !== presetId) return new Response(JSON.stringify({ ok: false, code: 'preset-changed', message: '当前预设已切换' }))
    if (body.promptConfigs) configs = body.promptConfigs
    for (const [key, entry] of Object.entries(body.overrides ?? {})) {
      if (entry === '' || Array.isArray(entry) && entry.length === 0) delete overrides[key]
  ...
  if (endpoint === 'engine-capability') {
    if (window.delayCapabilities) await new Promise(resolve => window.pendingCapabilities.push(resolve))
    if (body.expectedPresetId !== presetId) return new Response(JSON.stringify({ ok: false, code: 'preset-changed', message: '当前预设已切换' }))
    if (window.rejectCapability) return new Response(JSON.stringify({ ok: false, message: '能力创建失败' }))
    const ids = body.action === 'create-recipe' ? engineRecipe(body.recipeId)?.capabilities ?? [] : [body.capabilityId]
    for (const id of ids) for (const module of engineCapability(id)?.moduleKeys ?? []) {
      if (body.action === 'remove') modules.delete(module)
      else modules.add(module)
    }
    if (body.action === 'remove') for (const key of engineGroupParamKeys(body.capabilityId)) delete overrides[key]
    if (body.action === 'create-recipe') for (const [key, entry] of Object.entries(engineRecipe(body.recipeId)?.initialParams ?? {})) {
      if (!Object.hasOwn(overrides, key)) overrides[key] = entry
    }
    value = { changed: true }
  }
...
function App() {
  ...
  return React.createElement(React.Fragment, null,
    page === 'away' ? null : React.createElement(page === 'subagent' ? SubagentPage : MainSessionPage, { key: store.fields.presetTemplate, store, t }),
