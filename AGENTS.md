...
- 长任务：每完成一个 Wave 或累计 5 个原子任务，更新 `.ai-memory/handoff.md`（已完成项含验证证据 → 相关文件 → 未完成项与下一步 → 阻塞项）。
- 恢复：新会话涉及历史决策，或用户说“继续／下一步／接着做”时，先读当日与最近一次 `daily.md`，必要时读 `handoff.md`；文件缺失时静默继续，不阻塞任务。
