...
- 修复导入确认、转换报告、世界书诊断或会话角色完整性时，先读 [PLAN.md](PLAN.md) 的当前修复任务；计划归档只保留历史证据，不作为现行授权或完成状态。
- 消息出口遵守目标宿主的事件/角色契约：当前宿主 pre-step 仅允许 user，插件出口必须遵守。修复旧 assistant 配置时保留正文并明确降级告警，新转换不得宣称角色等价；策略返回 patch 同样经过共同出口校验。验收包含官方持久化后重新加载，不只断言内存消息。
- 预览确认绑定文件、选组、转换器版本与实际目标及其版本；任一变化先使旧预览失效，再同源重预览。后端拒绝非法预览参数和过期版本，摘要不能替代写入授权；等待确认时确认/取消必须可操作，卸载或换目标不得继续旧提交。
- 配置合并与报告共用最终 ID/来源映射；展示截断保留真实全量计数并明确提示。诊断快照在实际 commit 后仍同步 records/truncated；只读查看不再次运行选择器或宏，测试包含真实非空接线和边界交互。
- 防复发修复与历史日志恢复分别授权。日志恢复须核对匹配版本、静止窗口、不可变备份与内容版本，保留事件、seq 和引用，并经官方完整回放验证；不凭魔数拆帧或忽略解析失败判定恢复成功。
