...
- **迁移全部交给离线脚本（插件零运行时兼容）**：新增 `scripts/migrate-skills.mjs`（`pnpm migrate:skills`）——旧 per-profile 副本 `profiles/*/skills` 搬运到技能根、`settings.yaml` 里 `skillsDir`/`skillsDirs`/`skillSwitches`/`skillOrder`/`skillRankBase` → 配置文件 + 磁盘停用 + 删除旧键，`--clean-legacy` 把旧副本改名归档；`--dry-run` 只报告、写盘前备份、可重复运行（幂等）。废弃的 per-profile 路径在脚本里直接丢弃，不会重新挂成技能根。插件侧不留任何迁移代码与迁移状态（`skillsMigrated` 已移除）。
- **settings 参数兼容同样移到离线脚本**：旧全局 settings 引擎参数 → 各插件格式预设 `preset.yml` 的迁移从运行时移到 `scripts/migrate-presets.mjs`（复用插件的 `savePresetParams` 语义：模型段映射、空值删除、注释保留），搬完删除 settings 旧键；官方格式预设跳过。插件启动不再读写 `paramsMigrated` 状态，也不再自动迁移。
- **彻底去掉旧键**：settings schema 与 loader config 不再声明 `skillSwitches`/`skillOrder`/`skillsDirs`/`skillsDir`/`skillRankBase`（旧值只由脚本读取一次）；客户端 `Fields` 去掉 `skillSwitches`，启停状态直接取 `skillCatalog[].disabled`（TUI 同源）。
