...
#### 暂缓候选清单（不计入核心 Wave 完成度）

- [ ] **W4 / P2 世界书预算**：出现明确规模问题或用户需求后，再确认策略并单独拆解；尚未授权、未实施。
- [ ] **W5 / 声明式正则**：有真实资产样本且可满足安全执行边界后单独授权；未实施。
- [ ] **W5 / 条件宏**：按实际使用语义补充最小能力，先单独确认验收；未实施。
- [ ] **W5 / 变量持久化**：先确认存储所有权、恢复与冲突规则，再单独授权；未实施。
