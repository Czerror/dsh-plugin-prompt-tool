...
- 任务状态只维护在本文件；[AGENTS.md](AGENTS.md) 保存长期决策，领域文档保存实施后的稳定行为。
...
文档任务 D0 不计代码修复进度。未来只有用户明确要求实施后，才进入 R0–R6。

- [x] D0：新 PLAN、原文归档、AGENTS 决策及文档校验；提交/推送结果以 Git 记录和本次交付报告为准。
- [ ] Wave 1：R0、R1、R2，优先阻止新会话损坏和非预期写入。
  - [ ] R0：H0 合法消息出口、兼容降级与官方回放回归。
  - [ ] R1：F1 角色导入确认可结束。
  - [ ] R2：F7 导入请求类型校验。
- [ ] Wave 2：R3、R4、R5、R6，完成确认身份与诊断可信度。
  - [ ] R3：F2/F3 选组优先级与版本化预览闭环。
  - [ ] R4：F4 最终 ID 与来源报告同源。
  - [ ] R5：F5 全部有界有损项可查看。
  - [ ] R6：F6 commit 后诊断截断与真实接线。
- [ ] H1：历史会话恢复，独立授权，默认不执行。
...
- Task Summary 原地追加本节：任务状态、文件、证据、决策、偏差、遗留、下一步。源码未验证时不先勾选。
- 本地 daily.md 仅记录修改记忆，不放第二份任务账本、图谱或 handoff；本文件的状态和提交是恢复检查点。

### 4.4 Task Summary：D0 文档交付

- **状态**：文档编写与校验完成；R0–R6 未开始，H1 未授权。
- **文件**：PLAN.md、AGENTS.md、原文归档及 docs/ui-architecture.md 的计划入口。
- **验证**：归档与固定 Git blob 字节一致；3 份当前文档的 30 个本地链接有效；7 个 XML 任务字段完整、依赖无环；F1–F7、T00–T10 覆盖齐全；5 个命令中的现有测试路径与 package scripts 均有效；UTF-8 无 BOM、代码围栏及 diff --check 通过。
- **边界**：本次未运行业务测试/构建，不把审查阶段的 919/919 当作未来修复结果；未改用户数据、宿主源码或运行服务。
- **下一步**：文档提交/推送后停止；后续获得实施指令再从 R0 开始，历史日志恢复另行确认。
...
- 版本化修改仅新 PLAN、旧计划归档、AGENTS 长期决策，以及 UI 权威文档中失效的“当前无计划”入口文字。
...
完成文档、原文归档、长期规则与文档提交后停止。所有代码修复保持未开始，历史数据恢复保持待单独授权。
