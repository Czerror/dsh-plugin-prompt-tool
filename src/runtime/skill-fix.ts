...
export function fixSkillEntry(skillsDir: string, folder: string): SkillFixResult {
  ...
  const sourceDir = resolve(join(root, ...segments))
  ...
  const file = join(sourceDir, SKILL_MARKER)
  if (!existsSync(file)) {
    return { fixed: false, folder, fixedFolder: folder, name: folder, actions: [], error: 'SKILL.md 不存在，旧 .disabled 标记不受支持' }
  }
  ...
  const declaredName = typeof data.name === 'string' && data.name.length > 0 ? data.name : ''
  const nameValid = SKILL_NAME_RE.test(declaredName)
  const folderValid = segments.every((part) => SKILL_NAME_RE.test(part))
  const fixedFolder = folderValid ? folder : uniqueKebab(root, deriveKebab(folder, declaredName))
  const fixedName = fixedFolder.split('/').at(-1) ?? fixedFolder
  ...
  const targetFile = join(root, fixedFolder, SKILL_MARKER)
