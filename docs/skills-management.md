...
清单通过当前会话的 `SkillRegistry.snapshot({cwd, scope})` 标注生效、遮蔽、未注册或尚未确认；先按作用域覆盖，再按层内 rank、提供者和根顺序裁决。
...
- 「模型」和「用户」页签只收当前会话已确认生效且允许该端调用的技能；遮蔽、未注册和未确认条目保留在「全部」中。
  「两端不可用」按文件声明筛选两端关闭的技能；两个开关表示文件声明，徽章单独表示会话状态。
