...
管理页展示技能来源、文件调用策略和当前会话的注册状态，并提供文件编辑、创建、导入与可恢复删除。
...
- 只接受官方连字符键；旧 `disableModelInvocation` / `userInvocable` / `modelInvocable` 策略键使读取与写入失败，不自动转换，原文件保持不变；
...
- 只接受 `version: 4`；缺失或其他版本拒绝读写，不自动升级。当前版本中的未知字段保留原文且不参与行为。
- 写入走 yaml Document API：保留注释与未知字段，内容无变化时不落盘，失败清理暂存文件。
  `version` 只标识状态文件结构，不是技能内容版本。技能全文编辑另以原文 SHA-256 校验并发修改，不维护历史版本。
...
| 读取技能文件 | `/skill-read` | 提交 `{ name, path, sessionId? }`，返回 Markdown 正文 `content`、描述 `description` 与完整文件的 `revision`；展开可编辑技能时才读取 |
| 保存技能文件 | `/skill-write` | 提交 `{ name, path, content, description, expectedRevision, sessionId? }`；只替换指定文件，成功后局部刷新技能清单 |
| 添加 / 移除技能文件夹 | `/skills-folders` | 点击“引用文件夹”通过宿主选择目录，只登记路径；这些目录里的技能按 `custom` 优先级注册，源目录更新即时生效 |
| 创建技能 | `/skill-create` | 在 `$DSH_HOME/skills/<名称>/SKILL.md` 建标准 frontmatter，官方即刻发现 |
| 复制导入 | `/skills-import-directory` | 点击“导入技能”通过宿主选择单技能目录，复制进用户技能根；同名先提醒，用户确认后才覆盖；新旧目录均须含技能标记 |
...
技能目录操作只有“导入技能”和“引用文件夹”两个按钮，均使用宿主 `pickDirectory()`；页面不提供浏览器文件夹上传或手动路径输入。取消选择不提交，选择器失败显示错误，不自动回退其他入口；目录选择期间互斥，离开页面后忽略迟到结果。引用操作提交前读取最新引用列表，避免覆盖选择期间的列表刷新；失败保留列表事实，可以重新选择重试。底层 `/skills-import` 文件批次契约仍保留，但技能页不再调用。

技能卡收起时固定三行：名称与调用状态、单行省略描述、来源路径与优先级。可调用状态保留绿色圆点与胶囊。展开区不重复名称、描述和路径，从独立调用开关开始，随后为描述及 Markdown 正文编辑；正文输入不包含 YAML frontmatter，模型/用户名称在开关上方。底部为保存、重新读取及右对齐的删除按钮，空间不足时自动换行，不压缩按钮。来源类别在分组头部显示，不重复堆叠徽章。

编辑由服务端 `canEdit` 决定，支持当前工作区、用户及显式引用根中的普通文件；官方、系统、链接与只读目标不开放写入。保存通过 YAML Document 仅修改描述，保留原文件的名称、调用策略、注释和未知字段，再拼接 Markdown 正文。头部与正文之间的分隔空行不进入编辑器，保存时保留；正文内部空行和首行缩进原样保留。描述必填，完整文件上限为 1 MiB；缺失描述可补齐，损坏 YAML、别名或非法调用策略拒绝读写，不以空元数据覆盖原件。

预设与技能复用 `host/text-file.ts#atomicWriteTextFile`：同目录独占暂存、原子替换、失败清理；预设原入口继续转导出。来源授权、内容校验及保存后的重建或技能失效仍归各自领域。技能保存前核对读取版本，替换前再次核对清单身份与原文；外部修改返回冲突，原件不被覆盖。编辑器保留未保存草稿，重新读取前确认丢弃；有脏草稿时先保存或重新读取，再操作调用开关。此流程不重建预设，也不保存技能历史。
