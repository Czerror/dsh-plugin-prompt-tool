...
清单的同名遮蔽标记来自当前会话的官方注册表：先按作用域覆盖，再按层内 rank、提供者和根顺序裁决。
扫描器不按目录字典序或跨层 rank 猜测赢家；没有可用注册表时只展示文件事实。`/bootstrap` 与 `/skills-list`
使用同一个会话工作目录与作用域。TUI 的 `skill <技能名>` 使用 frontmatter.name；同名多个文件时要求到管理页选择具体条目。
...
- **候选缓存**收到文件事件即失效，同时清除引用目录扫描缓存；等长且同一时间戳粒度的改写也会刷新。
  其余官方技能根继续由官方文件提供者监听。读取时的根指纹仍用于无事件场景下发现新增、删除与普通改写。
