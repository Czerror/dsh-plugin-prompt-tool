...
工具栏的「添加能力 / 工具模块」是唯一创建入口，始终提供全部六层模板、工具和可添加能力，不按当前列表层级过滤。菜单内部平铺，不折叠二级入口；「添加模板 · 层级」打开该层模板浮层，另有「添加工具模板…」「添加模板变量」、新建空白工具、添加模块和连锁创建。创建只展开新卡，不动用户选的层级筛选与搜索词（目标卡落在被筛掉的层时保持不可见，切到该层或「全部」即可见）；模板重复创建时分配唯一标识，不覆盖原配置。工具模板选中后关闭浮层，创建意图绑定发起预设，不能切换预设后重放。空白工具的 id 和模型可见名均不重复。

世界书是提示词策略筛选，能力卡不混入该视图；自定义工具卡只在「全部」和「工具链」筛选下显示。筛选通过隐藏保留工具编辑器挂载，不卸载其未保存草稿；保存失败保留原输入。提示词搜索只作用于提示词配置，计数、批量启停和保存按钮不操作能力卡。

能力卡只展示当前预设 `modules` 显式声明的能力，一项已装配能力一张卡：标题是能力 id，卡头 meta 显示提供该能力的 `modules` 行，展开区直接编辑该能力自身的参数，不提供「编辑行为」这类编辑目标选择器；新能力仍从全量创建菜单添加。`modules: []` 不展开默认骨架，官方组合行不生成插件能力卡。二次确认删除只移除该能力的模块声明并保留 dormant 参数，成功后重建一次并刷新模块事实。
