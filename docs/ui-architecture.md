...
样式参照官方 `ui-settings-plugin-inventory/PluginInventorySettingsTab`，不是可配置插件表单。卡头复用共享 `StatusBadge`（8px 圆点 + 官方 Tag）与官方 Chevron，标记真实的「模型可见」；展开显示完整名称、来源视角、可见状态与描述。工具摘要没有插件配置启停或运行阶段，不显示虚构的「已启用／运行中」。搜索只在客户端过滤，并自动展开分组，不增加 bridge 请求。
...
- StatusBadge：只读状态徽章，8px 圆点 + 官方 Tag 胶囊；tone 同时驱动两者颜色，技能卡与工具预览共用。
...
主会话使用单一模块列表：公共配置（当前会话模型、模板变量和提示词生成默认值）位于列表顶部，工具栏提供插入点层级与策略筛选、合并创建菜单和提示词配置操作。列表平铺渲染提示词配置卡、已装配能力卡和自定义工具卡，不再按插入点分区块；下拉选中某个插入点层级时只过滤该层提示词配置与对应能力卡，展示仍是同一份平铺列表。提示词配置仍按（层序, order, 声明序）排序，`anchor-turn` 的实际 hook 不受展示影响。能力卡与提示词配置保留各自保存、排序和删除语义，不建立第二份字段状态。

工具栏的「添加能力 / 工具模块」是唯一创建入口：菜单内部平铺，不折叠二级入口；提示词模板按六个插入点层级展开为「添加模板 · 层级」，点击后浮层只列该层模板（无分组标题），另有「添加工具模板…」和「添加模板变量」，与新建空白工具、添加模块和连锁创建共用该菜单；自定义工具卡内不再保留独立新建按钮。世界书是提示词策略筛选，能力卡不混入该视图；自定义工具卡只在「全部」和「工具链」筛选下显示。提示词搜索只作用于提示词配置，计数、批量启停和保存按钮不操作能力卡。
