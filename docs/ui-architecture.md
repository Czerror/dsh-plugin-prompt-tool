...
指令文件（AGENTS.md / CLAUDE.md 及其 .local 变体）不走独立卡片：探测到的每个文件用**普通配置卡**（与 `example-pre-step` 同一个 PromptConfigCard）呈现在配置列表里，头部照常显示层级/策略/位置，展开后就地编辑正文，正文仍只经「保存到文件」显式写回原文件，绑定保持与官方 agent-instructions 等价（pre-step、真实用户消息之后、会话去重、随子代理晋升）。层级筛选、批量启停、层内移动沿用列表既有语义，不新增置顶卡片或层卡入口。
