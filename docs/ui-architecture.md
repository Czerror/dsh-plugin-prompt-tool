...
主会话使用单一模块列表：公共配置（当前会话模型、模板变量和提示词生成默认值）位于列表顶部，工具栏提供策略筛选、合并创建菜单和提示词配置操作。列表平铺渲染提示词配置卡、已装配能力卡和自定义工具卡，不再按插入点分区块；提示词配置仍按（层序, order, 声明序）排序，`anchor-turn` 的实际 hook 不受展示影响。能力卡与提示词配置保留各自保存、排序和删除语义，不建立第二份字段状态。

工具栏的「添加能力 / 工具模块」是唯一创建入口：从模板新建（提示词模板、工具模板、模板变量）、新建空白工具、添加模块和连锁创建共用该菜单；自定义工具卡内不再保留独立新建按钮。世界书是提示词策略筛选，能力卡不混入该视图。提示词搜索只作用于提示词配置，计数、批量启停和保存按钮不操作能力卡。
