...
子代理工具策略（`subagent-tool-policy`）是模块类型能力：在能力菜单里创建、在模块列表里有独立能力卡、用「删除引擎能力」移除；编辑器住在该能力卡内部（由页面经 `EngineModuleCards` 的 `renderCapabilityExtra` 插槽注入，「工具与深度」卡只留递归深度与入口提示）。该卡片只有**一个启用开关**：打开写入可用骨架并立即落盘（自动保存复用"焦点离开编辑区才写盘"的既有设计，没有保存按钮）；关闭删除顶层策略段并保留模块声明，同时把编辑区置为 `fieldset[disabled]` 只读；引擎在策略文件缺失时降级为官方委派行为，不阻止预设启动。启用能力时宿主一并写入顶层 `subagentToolPolicy` 骨架（模块在 ⇒ 数据在），删除能力时模块声明与顶层段一起移除；模块声明是唯一开关，顶层段不再隐式装配模块。子代理页排除「仅主对话」能力——不提供创建 `tool-filter`，也不渲染它的卡片，并在能力卡列表上方提示子代理工具面应走「subagent-tool-policy」能力。
