...
12. 「独立指令文件来源」总开关复用 ToggleRow，位于**前置步骤层卡**内（与它管辖的指令文件同处），只修改独立策略顶层 `enabled`，默认关闭；单文件开关不隐式开启总来源，也不改变官方负责人。策略不可读时禁用总开关；应答成功前不乐观显示已启用。
...
主会话使用单一模块列表，视觉顺序为：人设卡（脱离「公共配置」分组，不参与层过滤）→ 各层通用引擎行为卡 → 自定义工具卡 → 层级配置卡；公共配置（当前会话模型、模板变量和提示词生成默认值）位于列表顶部。视觉顺序不建立跨插入点的全局执行顺序，`anchor-turn` 的实际 hook 同样不受展示影响。工具栏提供插入点层级与策略筛选、合并创建菜单和提示词配置操作；列表筛选只影响展示，仍是同一份平铺列表，不按插入点分区块。能力与提示词配置保留各自保存、排序和删除语义，不建立第二份字段状态。

工具栏的「添加能力 / 工具模块」是唯一创建入口，始终提供全部六层模板、工具和可添加能力，不按当前列表层级过滤。菜单内部平铺，不折叠二级入口；「添加模板 · 层级」打开该层模板浮层，另有「添加工具模板…」「添加模板变量」、新建空白工具、添加模块和连锁创建。创建提示词配置后清空搜索并定位、展开该卡；模板重复创建时分配唯一标识，不覆盖原配置。工具创建切到工具链并展开新卡，工具模板选中后关闭浮层，创建意图绑定发起预设，不能切换预设后重放。空白工具的 id 和模型可见名均不重复。

世界书是提示词策略筛选，能力卡不混入该视图；自定义工具卡只在「全部」和「工具链」筛选下显示。筛选通过隐藏保留工具编辑器挂载，不卸载其未保存草稿；保存失败保留原输入。提示词搜索只作用于提示词配置，计数、批量启停和保存按钮不操作能力卡。

引擎行为卡只展示当前预设 `modules` 显式声明的能力，同层共用一张卡，通过「编辑行为」下拉选择已有行为的参数表单。下拉切换只是编辑目标切换，不修改装配或停用其他已添加行为；新能力仍从全量创建菜单添加。`modules: []` 不展开默认骨架，官方组合行不生成插件能力卡。二次确认删除只移除当前选中行为的模块声明并保留 dormant 参数，成功后重建一次并刷新模块事实。
...
指令文件（`$DSH_HOME/AGENTS.md` 与工作区 cwd→项目根链的 `AGENTS.md`/`CLAUDE.md`/`.local` 变体）不再作为列表里的独立卡片存在：探测到的每个文件成为**前置步骤层卡**里的一个条目，与同层引擎能力共用「编辑行为」下拉；选中后在该卡内编辑正文（正文编辑器与层卡一致）与独立策略字段。列表只渲染预设卡，计数与空状态仍按全量（文件本身就是生效的注入条目）。绑定保持与官方 `@deepseek-ai/dsh-agent-instructions` 等价：pre-step 层、真实用户消息之后、会话去重、随子代理晋升，卡内不提供插入点/填充来源选择器，只保留启停、层内序号与策略档位。
