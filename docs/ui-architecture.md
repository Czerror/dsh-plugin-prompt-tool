...
主会话使用单一模块列表，视觉顺序为：人设卡（脱离「公共配置」分组，不参与层过滤）→ 引擎能力卡 → 自定义工具卡 → 层级配置卡；公共配置（当前会话模型、模板变量和提示词生成默认值）位于列表顶部。该顺序只是视觉排序，不改变提示词配置按（层序, order, 声明序）的注入先后，`anchor-turn` 的实际 hook 同样不受展示影响。工具栏提供插入点层级与策略筛选、合并创建菜单和提示词配置操作；下拉选中某个插入点层级时只过滤该层提示词配置与对应能力卡，展示仍是同一份平铺列表，不按插入点分区块。能力卡与提示词配置保留各自保存、排序和删除语义，不建立第二份字段状态。
