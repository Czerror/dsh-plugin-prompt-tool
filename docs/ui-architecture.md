...
工具栏的「添加能力 / 工具模块」是唯一创建入口：从模板新建（提示词模板、工具模板、模板变量）、新建空白工具、添加模块和连锁创建共用该菜单；自定义工具卡内不再保留独立新建按钮。世界书是提示词策略筛选，能力卡不混入该视图；自定义工具卡只在「全部」和「工具链」筛选下显示。提示词搜索只作用于提示词配置，计数、批量启停和保存按钮不操作能力卡。
