...
模块卡（引擎行为卡、提示词生成默认值卡、自定义工具卡）与层级配置卡共用同一个 `configList` 列表容器与卡间距；world-book 视图只隐藏模块卡容器，不卸载工具草稿。引擎行为卡与工具卡默认折叠，只有创建/定位（工具栏「添加能力 / 工具模块」创建能力、新建或插入工具）才自动展开目标卡，其余展开与折叠完全由用户点击决定。只读预设（system 或关闭 `writePreset`）下工具卡同样默认折叠：卡头操作区与表单各自是独立的 `fieldset` 禁用边界，折叠按钮留在边界之外保持可点。
